...
if input == "check":
    ...
    print("p1 = "+str(p)+" p1^2 = "+str(p*p))

    # これのキー

    # 2^n-1なら、最短は2^(n/2)と2^(n/2+1)の大きさになるはず。桁の話。詳しい話は今後詰めていく。
    # 2^nと2nの話とかにも通ずるなら、合わせ技で行けなくもなさそう。
        ...
        if i < len(str(q)) or i < len(str(p)):
            if int(s[i]) > int(o[i]):
                ...
                if l != 0 and l > 0:
                    q = q + pow(10, l)
                    i = 1
            elif int(s[i]) < int(o[i]) and int(s[i-1]) > int(o[i-1]):
    ...
    print("\n i = "+str(i)+" l = "+str(l)+" p = "+str(p)+" q = "+str(q)+" o = "+o+" s-o = "+str(int(s)-int(p*q)))
    ...
    print("\n p = "+str(p)+"q = "+str(q)+" p*q = "+str(p*q)+" x = "+str(x))
